    ...
    if settings.is_cabinet_mode():
        ...
        if path:
            ...
            if url:
                ...
                # Explicit 'default'/'' means colorless and must not fall through to global style.
                section_style = section_cfg.get('style')
                if style:
                    resolved_style = None if style in ('default', '') else _resolve_style(style)
                elif section_style is not None:
                    resolved_style = None if section_style in ('default', '') else _resolve_style(section_style)
                else:
