...
def _resolve_rich_logo_url() -> str:
    ...
    try:
        mtime = int(Path(settings.LOGO_FILE).stat().st_mtime)
    except Exception:
        mtime = 0
    return f'{parsed.scheme}://{parsed.netloc}/cabinet/branding/bot-logo?v={mtime}'
...
async def build_main_menu_rich_html(user: User, texts, db: AsyncSession) -> str:
    """Собирает rich-HTML главного меню в стиле welcome-карточки."""
    ...
    balance = html.escape(settings.format_price(getattr(user, 'balance_kopeks', 0) or 0))
    ...
    try:
        ...
        if subscription:
            ...
            if tariff:
                if getattr(tariff, 'is_daily', False) and getattr(tariff, 'daily_price_kopeks', 0):
                    price = html.escape(settings.format_price(tariff.daily_price_kopeks))
        ...
        logger.debug('Не удалось собрать тариф для rich-меню', error=error)
    ...
    if hint_sections:
        ...
        blocks.append(f'<details open><summary>{pe("💡")} {html.escape(summary.lstrip("💡 ").strip())}</summary>{inner}</details>')
