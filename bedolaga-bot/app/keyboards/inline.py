...
def get_onboarding_consent_text(language: str = DEFAULT_LANGUAGE) -> str:
    from app.utils.premium_emoji import pe

    texts = get_texts(language)
    template = texts.t('ONBOARDING_CONSENT_TEXT', ONBOARDING_CONSENT_TEXT)
    return template.format(star=pe('⭐️'), doc=pe('📄'), lock=pe('🔒'))


def get_onboarding_consent_declined_text(language: str = DEFAULT_LANGUAGE) -> str:
    from app.utils.premium_emoji import pe

    texts = get_texts(language)
    template = texts.t('ONBOARDING_CONSENT_DECLINED_TEXT', ONBOARDING_CONSENT_DECLINED_TEXT)
    return template.format(warn=pe('❗️'))


def get_onboarding_consent_keyboard(language: str = DEFAULT_LANGUAGE) -> InlineKeyboardMarkup:
        ...
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text=texts.t('ONBOARDING_CONSENT_ACCEPT', 'Согласиться'),
                    callback_data='rules_accept',
                    style='success',
                    icon_custom_emoji_id=icon_id('accept') or None,
                ),
                InlineKeyboardButton(
                    text=texts.t('ONBOARDING_CONSENT_DECLINE', 'Отказаться'),
                    callback_data='rules_decline',
                    style='danger',
                    icon_custom_emoji_id=icon_id('decline') or None,
                ),
            ]
        ]
    )
...
def get_rules_keyboard(language: str = DEFAULT_LANGUAGE) -> InlineKeyboardMarkup:
    return get_onboarding_consent_keyboard(language)
    ...
    def _cabinet_button(
        ...
        *,
        ...
        if url:
            ...
            # 'default' / empty = colorless. Explicit default must NOT fall through
            # to global CABINET_BUTTON_STYLE (often 'success').
            section_style = section_cfg.get('style')
            if style:
                resolved = None if style in ('default', '') else _resolve_style(style)
            elif section_style is not None:
                resolved = None if section_style in ('default', '') else _resolve_style(section_style)
            else:
            ...
            # Premium icon renders separately — strip leading unicode to avoid doubles.
    ...
    for row_key in row_keys:
        ...
        for btn_id in btn_ids:
            ...
            if btn_id.startswith('custom_'):
                ...
                raw_style = custom_cfg.get('style')
                resolved_style = None if raw_style in ('default', '', None) else _resolve_style(raw_style)
...
def get_info_menu_keyboard(
    ...
    from app.utils.premium_emoji import clean_button_text, premium_button
    ...
    if show_promo_groups:
        buttons.append(
            [
                premium_button(
                    texts.t('MENU_PROMO_GROUPS_INFO', 'Промогруппы'),
                    icon='promo',
    ...
    if server_status_mode == 'external_link':
        ...
        if status_url:
            buttons.append(
                [premium_button(server_status_text, icon='status', url=status_url)]
            )
    ...
    buttons.append(
        [
            premium_button(
                clean_button_text(texts.BACK) or 'Назад',
                icon='back',
                callback_data='back_to_menu',
            )
        ]
...
def get_support_keyboard(language: str = DEFAULT_LANGUAGE) -> InlineKeyboardMarkup:
    ...
    if tickets_enabled:
        rows.append(
            [
                premium_button(
                    texts.t('CREATE_TICKET_BUTTON', 'Создать тикет'),
                    icon='ticket',
        ...
        rows.append(
            [
                premium_button(
                    texts.t('MY_TICKETS_BUTTON', 'Мои тикеты'),
                    icon='tickets',
    ...
    if contact_enabled and settings.get_support_contact_url():
        rows.append(
            [
                premium_button(
                    texts.t('CONTACT_SUPPORT_BUTTON', 'Написать в поддержку'),
                    icon='contact',
