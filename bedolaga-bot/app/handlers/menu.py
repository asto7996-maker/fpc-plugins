...
async def get_main_menu_text(user, texts, db: AsyncSession):
    """LuxuryVPN-style welcome profile card with premium tgmacicons emojis."""
    from app.config import settings
    from app.utils.premium_emoji import inject_premium_emojis, pe

    balance = settings.format_price(getattr(user, 'balance_kopeks', 0) or 0)
    tariff_line = texts.t('MAIN_MENU_TARIFF_NONE', 'не выбран')
    ...
    try:
        ...
        if subscription:
            ...
            if tariff:
                if getattr(tariff, 'is_daily', False) and getattr(tariff, 'daily_price_kopeks', 0):
                    price = settings.format_price(tariff.daily_price_kopeks)
                    tariff_line = texts.t(
                        'MAIN_MENU_TARIFF_DAILY',
                        '{name} — {price} / день',
                    ).format(name=html.escape(tariff.name), price=price)
                else:
                    tariff_line = html.escape(tariff.name)
            elif getattr(subscription, 'is_trial', False):
                tariff_line = texts.t('MAIN_MENU_TARIFF_TRIAL', 'Тестовая подписка')
            else:
                tariff_line = texts.t('MAIN_MENU_TARIFF_ACTIVE', 'Активна')
    except Exception as error:
        logger.debug('Не удалось собрать тариф для главного меню', error=error)
    ...
    return inject_premium_emojis(base_text)
...
class MockMessage:
    def __init__(self, message: types.Message):
        ...
        # text=None (не "") чтобы try_edit_rich_main_menu пошёл по пути
        # «новое сообщение», а не «редактировать пользовательское сообщение»
        self.text = None
        self.caption = ""
        self.photo = None
    ...
    async def edit_text(self, text, reply_markup=None, *args, **kwargs):
        return await self._message.answer(text, reply_markup=reply_markup, *args, **kwargs)

    async def edit_caption(self, caption, reply_markup=None, *args, **kwargs):
        return await self._message.answer(caption, reply_markup=reply_markup, *args, **kwargs)

    async def edit_media(self, media, reply_markup=None, *args, **kwargs):
...
async def handle_reply_subscription(message: types.Message, db_user: User, db: AsyncSession):
    mock_cb = MockCallbackQuery(message)
    await show_main_menu(mock_cb, db_user, db, skip_callback_answer=True)


async def handle_reply_info(message: types.Message, db_user: User, db: AsyncSession):
    mock_cb = MockCallbackQuery(message)
    await show_info_menu(mock_cb, db_user, db)


async def handle_reply_support(message: types.Message, db_user: User, db: AsyncSession):
    from app.handlers.support import show_support_info
    mock_cb = MockCallbackQuery(message)
    await show_support_info(mock_cb, db_user, db)


def register_handlers(dp: Dispatcher):
    dp.message.register(
        handle_reply_subscription,
        StateFilter(None),
        F.text.in_(
            [
                'Меню',
