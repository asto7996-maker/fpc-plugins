...
# ──────────────────────────────────────────────────────────────────────────────
#  Gemini Assistant — отзывы ИИ + автозакупка и ссылки FunPay
# ──────────────────────────────────────────────────────────────────────────────
...
VERSION       = "3.8.0"
DESCRIPTION   = "ИИ-отзывы + автозакупка gemini_18m и ссылки FunPay — всё в одном"
CREDITS       = "Cursor AI"
UUID          = "c4e8b2f1-9a3d-4e7b-8c6f-2d1a5e9b0c3f"
SETTINGS_PAGE = True
BIND_TO_DELETE = None

_LINK_UUID: Final[str] = "f7a2e8c1-4b3d-4e9f-a8c2-1d5e9b0f6a3c"
...
class Plugin:
    ...
    @staticmethod
    def _migrate_from_link_plugin(cfg: dict[str, Any]) -> bool:
        """Перенос настроек/склада из отдельного gemini_link.py (v2.x)."""
        link_dir = f"storage/plugins/{_LINK_UUID}"
        link_settings = f"{link_dir}/settings.json"
        if not os.path.exists(link_settings):
            return False
        stock_keys = (
            "autobuy_enabled", "autobuy_quantity", "autobuy_lot_match", "autobuy_lot_id",
            "supplier_api_url", "supplier_api_key", "supplier_product_id", "supplier_product_name",
            "supplier_mode", "supplier_bulk", "supplier_parallel", "account_line_template",
            "import_routes", "import_line_template", "import_skip_duplicates", "imported_order_ids",
            "stock_mode", "warehouse_release_qty", "autobuy_auto_restock", "autobuy_min_lot_stock",
        )
        changed = False
        try:
            with open(link_settings, "r", encoding="utf-8") as f:
                link = json.load(f)
            for key in stock_keys:
                link_val = link.get(key)
                if link_val in (None, "", []):
                    continue
                cur = cfg.get(key)
                if cur in (None, "", []) or (key == "supplier_product_id" and not cur):
                    cfg[key] = link_val
                    changed = True
            link_wh = f"{link_dir}/warehouse.json"
            if os.path.exists(link_wh) and not os.path.exists(WAREHOUSE_FILE):
                os.makedirs(os.path.dirname(WAREHOUSE_FILE), exist_ok=True)
                shutil.copy2(link_wh, WAREHOUSE_FILE)
                changed = True
        except Exception as exc:
            logger.warning("%s link plugin migration: %s", _P, exc)
        return changed
    ...
    def reload_settings(self) -> None:
        ...
        try:
            if os.path.exists(SETTINGS_FILE):
                ...
                migrated = False
                if old_ver < "3.8.0":
                    migrated = self._migrate_from_link_plugin(loaded)
                    if not loaded.get("supplier_product_id"):
                        loaded["supplier_product_id"] = "gemini_18m"
                    loaded["_cfg_version"] = "3.8.0"
                    if migrated:
                        self.log("настройки перенесены из Gemini Link Auto")
                else:
                    loaded.setdefault("_cfg_version", "3.8.0")
                self._cfg = loaded
                if old_ver < "3.8.0" or migrated:
                    self._save_settings()
            else:
                self._cfg = defaults
                if self._migrate_from_link_plugin(self._cfg):
                    self.log("настройки перенесены из Gemini Link Auto")
                self._save_settings()
        except Exception as exc:
    ...
    def setup_telegram(self) -> None:
        ...
        tg.msg_handler(send_panel, func=lambda m: _tg_matches(m, "gemini", "gemini_link", "gl"))
        tg.msg_handler(send_stock_cmd, func=lambda m: _tg_matches(m, "gl_stock"))

        self.log("Telegram UI: /gemini, /gemini_link, /gl_stock")
