...
class Plugin:
    ...
    def start_import_mode(self, chat_id: int, user_id: int, target: str = "lot") -> str:
        ...
        return (
            ...
            "Ссылки накапливаются; через ~2 с после последнего сообщения — предпросмотр.\n"
            "<code>/done</code> — разобрать сейчас, <code>/cancel</code> — отмена"
        )

    def _count_import_urls(self, chat_id: int, user_id: int) -> int:
        raw = self._import_buffers.get((chat_id, user_id), "")
        items = parse_shop_purchase_history(raw)
        if items:
            return len([it for it in items if str(it.get("url", "")).strip()])
        return len(extract_activation_urls(raw))

    def _cancel_import_finalize(self, chat_id: int, user_id: int) -> None:
        key = (chat_id, user_id)
        timer = self._import_finalize_timers.pop(key, None)
        if timer:
            timer.cancel()

    def _schedule_import_finalize(self, chat_id: int, user_id: int) -> None:
        self._cancel_import_finalize(chat_id, user_id)
        key = (chat_id, user_id)

        def _finalize() -> None:
            self._import_finalize_timers.pop(key, None)
            if key not in self._import_buffers:
                return
            if not self._import_buffer_has_urls(chat_id, user_id):
                return
            tg = self.cardinal.telegram
            if tg:
                tg.clear_state(chat_id, user_id)
            _boot_log(
                f"IMPORT debounced finalize chat={chat_id} user={user_id} "
                f"urls={self._count_import_urls(chat_id, user_id)}"
            )
            self.process_import_buffer(chat_id, user_id)

        timer = threading.Timer(IMPORT_FINALIZE_SEC, _finalize)
        timer.daemon = True
        timer.name = f"GlImportFinalize-{chat_id}"
        self._import_finalize_timers[key] = timer
        timer.start()
    ...
    def _setup_telegram_handlers(self) -> None:
        ...
        def on_import_text(message: Message) -> None:
            ...
            if chat_id in pl._import_pending:
                bot.reply_to(
                    message,
                    "⚠️ Сначала подтвердите или отмените текущий импорт (кнопки в сообщении выше).",
                )
                return
            ...
            if low in ("/done", "готово", "done"):
                pl._cancel_import_finalize(chat_id, uid)
                tg.clear_state(chat_id, uid)
            ...
            url_count = pl._count_import_urls(chat_id, uid)
            if pl._import_buffer_has_urls(chat_id, uid):
                pl._schedule_import_finalize(chat_id, uid)
            url_hint = f", ссылок: <b>{url_count}</b>" if url_count else ""
            bot.reply_to(
                message,
                f"✅ Принято (+{len(text)} симв., всего <b>{added}</b>{url_hint})\n"
                f"Отправьте ещё ссылки, <code>/done</code> — разобрать сейчас",
                parse_mode="HTML",
            )
