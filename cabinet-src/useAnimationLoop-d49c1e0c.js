...
/* smooth-v4: 90–144 FPS, pause canvas RAF while scrolling (not on tap) */
...
function y(s,f){
  ...
  r.useEffect(()=>{
    ...
    const setScrollClass=on=>{
      try{document.documentElement.classList.toggle("is-scrolling",!!on)}catch{}
    };
    const tick=l=>{
      if(busy()){e=0;return}
      const m=l-n;
    ...
    const markScroll=()=>{
      ...
      scrollTimer=setTimeout(()=>{scrolling=!1;setScrollClass(!1);start()},140);
    };
