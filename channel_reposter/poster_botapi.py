"""
poster_botapi.py — перезалив только через Bot API.

Нужно:
  1) Добавить бота админом в канал-источник (права на сообщения);
  2) Добавить бота админом в канал-назначение (публикация);
  3) Указать источник, назначение и стартовую ссылку на пост.

Копирование идёт через copyMessage — без метки «Forwarded from»,
подпись заменяется на ваш шаблон (HTML / форматирование из Telegram).
"""
