...
def format_support(settings: Settings) -> str:
    return (
        ...
        f"Опишите, что происходит: какое устройство, какое приложение "
        f"и на каком шаге возникла проблема — так мы ответим быстрее. "
        f"Если вопрос про оплату, укажите время платежа и способ.\n\n"
        f"{bullet('Мы на связи и отвечаем в течение дня')}\n"
        f"{bullet('Частые вопросы и документы — в разделе «ℹ️ Инфо»')}"
    )
