...
Live build `pk-build:20260812-fix`: FX-only injection: no overscroll/touchmove hijack, no broad style overrides, scroll events only pause orb CSS, `touch-action: manipulation`, duplicate React portal backgrounds disabled via `__paskodDisableBg`.
